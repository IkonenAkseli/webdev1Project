 ...
 *
 * @param {http.incomingMessage} request a response
...
const getCredentials = request => {
  ...
  const slicedText = encodedAuth.substring(6);
  const textEncoded = Buffer.from(slicedText, 'base64');
  const text = textEncoded.toString('ascii');
  
  
  const decodedArr = text.split(':');
  return decodedArr;
};
